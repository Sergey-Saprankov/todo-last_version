import {v1} from "uuid";
import {ActionType, FilterTaskType, TodoEntityType} from "./redux-type/redux-type";
...
export const todoListReducer = (state: TodoEntityType[] = initialState, action: ActionType): TodoEntityType[] => {
    switch (action.type) {
        case "SET-TODOS":
            return action.todos;
        case "ADD-TODO-LIST":
        ...
        case "CHANGE-TODO-LIST-TITLE":
            return [...state.map(t => t.id === action.todoListId ? {...t, todoListTitle: action.todoListTitle} : t)];
        default:
            return state;
    }
};
